...
where t.xtype='U' order by t.name asc";
        ...
        private void DBT_Btn_Click(object sender, EventArgs e)
        {
            ...
            string NameSpaceStr_DBU = this.NameSpaceDBU_Box.Text;
            ...
            {
                DBS_Control DBSC = new DBS_Control(this.DBConnString, NameSpaceStr_DBO, OutDirStr_DBO, NameSpaceStr_DAT, OutDirStr_DAT, this.DBTabList.CheckedItems[i].ToString(), NameSpaceStr_DBU, ConnectionMark, UserReadOnlyForSelect);
                DBSC.CreateAll();
        ...
        private void Combo_Conn_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (this.Combo_Conn.SelectedIndex != -1 && !string.IsNullOrEmpty(this.Combo_Conn.Text))
            {
                ...
                if (!string.IsNullOrEmpty(ConfigurationManager.AppSettings[this.Combo_Conn.Text + "_NAS_DBU"]))
                {
                    this.NameSpaceDBU_Box.Text = ConfigurationManager.AppSettings[this.Combo_Conn.Text + "_NAS_DBU"];
                }
