...
namespace Lixx.DBS
{
    public class DBS_Control
    {
        ...
        public DBS_Control(string ConnStr, string NameSpaceHead_DBO, string FolderHead_DBO, string NameSpaceHead_DAT, string FolderHead_DAT, string TableName, string NameSpaceHead_DBU, string ConnectionMark, bool UseReadOnlyForSelect = true)
        {
            ...
            this._DBUNameSpace = NameSpaceHead_DBU;
...
@"select a.*,b.value as Description,c.name as Xtype_Name,comm.text as dval from 
(
...
left outer join syscomments as comm on a.cdefault = comm.id";
            ...
            if (SDT != null && SDT.Rows.Count > 0)
            {
                ...
                //创建数据访问类目录
                this.CheckAndCreateDir(RootFolder_DBO + "\\DBOper");
                //创建数据实体类
                this.Create_Class(SDT, this._TableName, RootFolder_DAT + "\\" + this._TableName + ".cs", this._ModelNameSpace, this._ModelFolder);

                //创建数据访问类
                this.Create_DBOper(SDT, this._TableName, RootFolder_DBO + "\\DBOper\\" + this._TableName + ".cs", this._DBONameSpace, this._DBOFolder, this._ModelNameSpace, this._DBUNameSpace, this._UseReadOnlyForSelect);
            }
        }
        /// <summary>
        /// 创建实体文件
        ...
        private void Create_DBOper(DataTable dt, string tableName, string aimFile, string namespaceHead_dbo, string dbofolderHead, string namespaceHead_class, string namespaceHead_dbu, bool UseReadOnlyForSelect)
        {
            DBS_CCDBOper DBSCCD = new DBS_CCDBOper(dt, namespaceHead_dbo, dbofolderHead, tableName, namespaceHead_class, namespaceHead_dbu, UseReadOnlyForSelect);
